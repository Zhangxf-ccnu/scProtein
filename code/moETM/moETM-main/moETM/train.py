...
def Train_moETM_for_cross_prediction(trainer, Total_epoch, train_num, batch_size, Train_set, Test_set, save_dir):
    LIST = list(np.arange(0, train_num))
    ...
    for epoch in range(Total_epoch):
        ...
        if (epoch % 10 == 0) or (epoch == Total_epoch - 1):
            ...
            if trainer.direction == 'rna_to_another':
                recon_mod = np.array(recon_mod2.cpu().detach()) * test_mod2_sum[:, np.newaxis]
                gt_mod = np.array(test_X_mod2.cpu().detach()) * test_mod2_sum[:, np.newaxis]


            elif trainer.direction == 'another_to_rna':
                recon_mod = np.array(recon_mod1.cpu().detach()) * test_mod1_sum[:, np.newaxis]
                gt_mod = np.array(test_X_mod1.cpu().detach()) * test_mod1_sum[:, np.newaxis]

            else:
                print('Wrong Direction!')

            ### save impute results
            if (epoch%100==0) or (epoch == Total_epoch - 1):
                np.save(save_dir+"/recon_mod_epoch"+str(epoch)+'.npy',recon_mod)
                sio.savemat(save_dir+"/recon_mod_epoch"+str(epoch)+'.mat',{'recon':recon_mod})
                np.save(save_dir+"/gt_mod_epoch"+str(epoch)+'.npy',gt_mod)
                sio.savemat(save_dir+"/gt_mod_epoch"+str(epoch)+'.mat',{'gt':gt_mod})
