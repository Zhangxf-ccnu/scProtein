# -*- coding: utf-8 -*-
"""
Created on Tue Sep  5 09:20:24 2023
...
def plot_scatter_with_r(
    ...
    if one_to_one and (xlim is not None or ylim is not None):
        assert xlim == ylim
    if xlim:
        keep_idx = ensure_arr((x >= xlim[0]).multiply(x <= xlim[1]))
        x = ensure_arr(x[keep_idx])
        y = ensure_arr(y[keep_idx])
    if ylim:
        keep_idx = ensure_arr((y >= ylim[0]).multiply(x <= xlim[1]))
        x = ensure_arr(x[keep_idx])
        y = ensure_arr(y[keep_idx])
    ...
    if subset > 0 and subset < x.size:
        ...
        x = ensure_arr(x[indices])
        y = ensure_arr(y[indices])
...
# Model
    ...
    for h_dim,lr,rand_seed in param_combos:
        ...
        modal_dataset.data = ensure_arr(modal_dataset.data)
        modal_dataset.data = clr_transform(modal_dataset.data)
        sc_protein=torch.Tensor(modal_dataset.data)

        test_modal_dataset.data = ensure_arr(test_modal_dataset.data)
